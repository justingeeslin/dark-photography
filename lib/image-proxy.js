...
module.exports = function () {
  ...
  app.get('/:url/:width/:height.:extension?', function (req, res, next) {
    ...
    if (parseInt(width) > 10000) {
      return res.status(404).send('Expected width to be less than or equal to 10,000');
    }
    if (isNaN(parseInt(height))) {
      return res.status(404).send('Expected height to be an integer');
    }
    if (parseInt(height) > 10000) {
      return res.status(404).send('Expected height to be less than or equal to 10,000');
    }
